...
> 2.  **关于费用**：Cloudflare Zero Trust 拥有非常慷慨的 **免费套餐**（支持 50 个用户），完全能够满足个人或小团队需求。**但是，首次开通时系统可能要求绑定支付方式（支持 PayPal 或信用卡）才可以开始使用**。  
> 请放心，它是包月制的，它明确写了零美金一个月，而不是随用随付的，所以你只要别一开始选错了还不检查一路继续到结束就好，它是肯定不会扣费的。   
> 估计是为了防止滥用吧，毕竟这玩意真好用吧
